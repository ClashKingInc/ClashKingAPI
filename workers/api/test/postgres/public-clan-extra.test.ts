...
describe("public clan extra reads against authoritative Goose migrations", () => {
  it("encodes all five public routes from actual cached, ranking and CWL producer rows", async () => {
    await Effect.runPromise(Effect.gen(function* () {
      ...
      const hugeRounds = [["#CLANWAR", "#" + "P".repeat(8 * 1024 * 1024)]]
      yield* sql`UPDATE cwl_groups SET rounds = ${JSON.stringify(hugeRounds)}::jsonb WHERE cwl_id = ${groupId}`
      expect(yield* run(`/cwl/${encodeURIComponent(clanTag)}/group?season=2026-09`).pipe(Effect.flip)).toMatchObject({ _tag: "UpstreamUnavailable" })
      expect(get).toHaveBeenCalledTimes(1)
