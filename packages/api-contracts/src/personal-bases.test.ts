...
describe("personal base contracts", () => {
  it("publishes authenticated list, save/relabel, unsave, and cleanup routes", () => {
    expect(PersonalBasesEndpoint).toMatchObject({ auth: "user", method: "GET", path: "/v2/bases/personal" })
    expect(SavePersonalBaseEndpoint).toMatchObject({ auth: "user", method: "PUT", path: "/v2/bases/personal/:baseId", bodyMode: "json" })
  ...
  it("accepts only an explicit nullable War or Legend kind", () => {
    expect(Schema.decodeUnknownSync(SavePersonalBaseEndpoint.body)({ kind: "war" })).toEqual({ kind: "war" })
    expect(Schema.decodeUnknownSync(SavePersonalBaseEndpoint.body)({ kind: null })).toEqual({ kind: null })
    expect(() => Schema.decodeUnknownSync(SavePersonalBaseEndpoint.body)({ kind: 1 })).toThrow()
    expect(() => Schema.decodeUnknownSync(SavePersonalBaseEndpoint.body)({ kind: null, number: 1 })).toThrow()
  })
  ...
  it("keeps base IDs as decimal strings and returns one items collection without slots", () => {
    const state = { items: [{ id: "9223372036854775807", baseLink: "https://link.clashofclans.com/en?action=OpenLayout&id=TH17",
      ...
      downloadCount: 2, upvotes: 1, downvotes: 0, kind: "war" as const, saved: true,
