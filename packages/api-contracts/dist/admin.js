...
const IntBetween = (minimum, maximum) => Schema.Int.check(Schema.isBetween({ minimum, maximum }));
const UUID = Schema.String.check(Schema.isPattern(/^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/iu));
...
export const LabSendInput = Schema.Struct({
    device_ids: Schema.Array(Schema.String.check(Schema.isPattern(/^[a-f0-9]{32}$/u))).check(Schema.isMinLength(1), Schema.isMaxLength(25)),
    title: NonEmptyString.check(Schema.isMaxLength(240)),
    body: NonEmptyString.check(Schema.isMaxLength(2_000)),
    data: StringMap,
});
