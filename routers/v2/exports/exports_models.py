...
class WarStatsExporter:
        ...
        for hit in self.war_hits:
            ...
            if own_th != "Unknown" and enemy_th != "Unknown":
                ...
                if matchup_key not in self.attack_matchups:
                    self.attack_matchups[matchup_key] = {0: 0, 1: 0, 2: 0, 3: 0, 'total_destruction': 0,
                                                         'attack_count': 0}
                ...
                if defense_key not in self.defense_matchups:
                    self.defense_matchups[defense_key] = {0: 0, 1: 0, 2: 0, 3: 0, 'total_destruction': 0,
                                                          'attack_count': 0}
    ...
    def _get_summary_data(self) -> List[Tuple[str, Any]]:
        ...
        return [
            ...
            # War Stats
            ["CWL Attacks",
             f"{cwl_attacks} ({round(cwl_attacks / self.total_attacks * 100, 2)}%)" if self.total_attacks > 0 else "0 (0%)"],
            ["Regular War Attacks",
             f"{regular_attacks} ({round(regular_attacks / self.total_attacks * 100, 2)}%)" if self.total_attacks > 0 else "0 (0%)"],
            ["Friendly War Attacks",
             f"{friendly_war_attacks} ({round(friendly_war_attacks / self.total_attacks * 100, 2)}%)" if self.total_attacks > 0 else "0 (0%)"],
            ["", ""],  # Separator
            ...
            ["2 Star Attacks",
             f"{self.two_stars} ({round(self.two_stars / self.total_attacks * 100, 2)}%)" if self.total_attacks > 0 else "0 (0%)"],
            ["1 Star Attacks",
             f"{self.one_star} ({round(self.one_star / self.total_attacks * 100, 2)}%)" if self.total_attacks > 0 else "0 (0%)"],
            ...
            # Destruction Analysis
            ["High Destruction (≥90%)",
             f"{high_destruction} ({round(high_destruction / self.total_attacks * 100, 2)}%)" if self.total_attacks > 0 else "0 (0%)"],
            ["Medium Destruction (50-89%)",
             f"{medium_destruction} ({round(medium_destruction / self.total_attacks * 100, 2)}%)" if self.total_attacks > 0 else "0 (0%)"],
            ["Low Destruction (<50%)",
             f"{low_destruction} ({round(low_destruction / self.total_attacks * 100, 2)}%)" if self.total_attacks > 0 else "0 (0%)"],
            ["", ""],  # Separator

            # Attack Type
            ["Fresh Attacks",
             f"{fresh_attacks} ({round(fresh_attacks / self.total_attacks * 100, 2)}%)" if self.total_attacks > 0 else "0 (0%)"],
            ["Cleanup Attacks",
             f"{cleanup_attacks} ({round(cleanup_attacks / self.total_attacks * 100, 2)}%)" if self.total_attacks > 0 else "0 (0%)"]
        ]
    ...
    def _add_attack_performance_matrix(self, ws_summary):
        ...
        for (your_th, enemy_th) in sorted(self.attack_matchups.keys(), key=lambda x: (
            ...
            if total_matchup_attacks > 0:
                ...
                avg_destruction = stars_data.get('total_destruction',
                                                 0) / total_matchup_attacks if total_matchup_attacks > 0 else 0
    ...
    def _add_defense_performance_matrix(self, ws_summary):
        ...
        for (enemy_th, your_th) in sorted(self.defense_matchups.keys(), key=lambda x: (
            ...
            if total_defense_attacks > 0:
                ...
                avg_destruction_given = stars_data.get('total_destruction',
                                                       0) / total_defense_attacks if total_defense_attacks > 0 else 0
        ...
        format_table(ws_summary, defense_table_start, defense_table_end, "DefenseMatrix",
