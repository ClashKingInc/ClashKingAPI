...
async def export_cwl_summary_to_excel(tag: str):
    ...
    try:
        ...
        # Add logo
        await insert_logo_from_cdn(ws,
                                   "https://assets.clashk.ing/logos/crown-text-white-bg/BqlEp974170917vB1qK0zunfANJCGi0W031dTksEq7KQ9LoXWMFk0u77unHJa.png",
                                   "J1", 50)

        # Title
        ws[
            "A1"] = f"CWL Summary for {cwl_data.get('clan_name', 'Unknown')} - Season {cwl_data.get('season', 'Unknown')}"
        ...
        clean_clan_name = "".join(
            c for c in cwl_data.get('clan_name', 'Clan') if c.isalnum() or c in (' ', '-', '_')).rstrip()
...
async def export_player_war_stats(filter: PlayerWarhitsFilter):
    ...
    def get_filter_summary():
        ...
        if filter.timestamp_start != 0 or filter.timestamp_end != 2527625513:
            start_date = datetime.fromtimestamp(filter.timestamp_start).strftime(
                '%Y-%m-%d') if filter.timestamp_start > 0 else "Beginning"
            end_date = datetime.fromtimestamp(filter.timestamp_end).strftime(
                '%Y-%m-%d') if filter.timestamp_end < 2527625513 else "Present"
