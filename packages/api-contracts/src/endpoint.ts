...
export interface ErrorResponseSpec<
  ...
  Body extends ContractSchema = ContractSchema,
> {
  readonly body: Body
...
export interface Endpoint<
  ...
  Query extends ContractSchema,
  Body extends ContractSchema,
  Response extends ContractSchema,
  Errors extends ReadonlyArray<ErrorResponseSpec> = readonly [],
> {
...
export const defineEndpoint = <
  ...
  Body extends ContractSchema,
  Response extends ContractSchema,
  const Errors extends ReadonlyArray<ErrorResponseSpec> = readonly [],
>(
  endpoint: Endpoint<PathParams, Query, Body, Response, Errors>,
): Endpoint<PathParams, Query, Body, Response, Errors> => endpoint
...
export type EndpointErrorResult<E extends AnyEndpoint> =
  EndpointErrorSpec<E> extends infer Spec
    ? Spec extends ErrorResponseSpec<infer Status, infer Body>
      ? {
...
export type EndpointStatusResult<E extends AnyEndpoint> =
  | { readonly ok: true; readonly status: number; readonly value: EndpointResponse<E> }
