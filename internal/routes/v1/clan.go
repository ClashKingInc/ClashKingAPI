...
func clanBasic(a apptypes.Deps) fiber.Handler {
	return func(c *fiber.Ctx) error {
		row, err := v1BasicClan(c, a, clanFixTag(c.Params("clan_tag")))
		if err != nil {
...
func clanJoinLeave(a apptypes.Deps) fiber.Handler {
	return func(c *fiber.Ctx) error {
		start := time.Unix(clanParseInt64Default(c.Query("timestamp_start"), 0), 0).UTC()
		end := time.Unix(clanParseInt64Default(c.Query("time_stamp_end"), 9999999999), 0).UTC()
		...
		`, clanFixTag(c.Params("clan_tag")), start, end)
		if err != nil {
			return err
		}
		defer rows.Close()
...
func clanSearch(a apptypes.Deps) fiber.Handler {
	return func(c *fiber.Ctx) error {
		limit := clamp(clanParseIntDefault(c.Query("limit"), 100), 1, 500)
		...
		if locationID != "" {
			...
			args = append(args, clanParseIntDefault(locationID, 0))
		}
