...
func getUserGuilds(a apptypes.Deps) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		result := make([]models.GuildInfo, 0, len(adminGuilds))
		for _, g := range adminGuilds {
			hasBot := a.Discord.IsInGuild(c.UserContext(), int64(g.ID))
...
func buildGuildInfo(g discord.OAuth2Guild, hasBot bool) models.GuildInfo {
	mc := g.ApproximateMemberCount
	return models.GuildInfo{
		ID:          g.ID.String(),
		Name:        g.Name,
		Icon:        guildIconURL(g),
		Owner:       g.Owner,
