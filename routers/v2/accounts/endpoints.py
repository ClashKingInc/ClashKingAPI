...
async def add_coc_account(request: CocAccountRequest, user_id: str = Depends(get_current_user_id), *, mongo: MongoClient):
...
@linkd.ext.fastapi.inject
@capture_endpoint_errors
async def add_coc_account_with_verification(request: CocAccountRequest, user_id: str = Depends(get_current_user_id), *, mongo: MongoClient):
    ...
    if old_account:
        ...
        remaining_accounts = await mongo.coc_accounts.find({"user_id": old_user_id}).sort("order_index", 1).to_list(
            length=None)
...
@linkd.ext.fastapi.inject
@capture_endpoint_errors
async def get_coc_accounts(user_id: str = Depends(get_current_user_id), *, mongo: MongoClient):
...
@linkd.ext.fastapi.inject
@capture_endpoint_errors
async def remove_coc_account(request: CocAccountRequest, user_id: str = Depends(get_current_user_id), *, mongo: MongoClient):
    ...
    remaining_accounts = await mongo.coc_accounts.find({"user_id": user_id}).sort("order_index", 1).to_list(
        length=None)
...
@linkd.ext.fastapi.inject
@capture_endpoint_errors
async def reorder_coc_accounts(request: dict, user_id: str = Depends(get_current_user_id), *, mongo: MongoClient):
...
@linkd.ext.fastapi.inject
@capture_endpoint_errors
async def verify_coc_account(request: CocAccountRequest, user_id: str = Depends(get_current_user_id), *, mongo: MongoClient):
