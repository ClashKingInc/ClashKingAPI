...
@router.patch("/roster-templates/{template_id}", response_model=dict, status_code=status.HTTP_200_OK, name="Update Roster Template")
async def update_roster_template(template_id: str, payload: TemplateUpdateModel):
    ...
    allowed = {k: body[k] for k in ("alias", "server_id", "th_restriction", "active", "frequency", "event_time", 
                                   ...
                                   "auto_publish", "auto_close_registration", "auto_publish_results", "clan_tag") if k in body}
...
@router.get("/roster-templates/{template_id}", response_model=dict, status_code=status.HTTP_200_OK, name="Get Roster Template")
...
@router.delete("/roster-templates/{template_id}", response_model=dict, status_code=status.HTTP_200_OK, name="Delete Roster Template")
...
@router.post("/rosters/{roster_id}/members", response_model=dict, status_code=status.HTTP_201_CREATED, name="Add Members by Tags (Auto-Fetch Data)")
async def add_members_to_roster(roster_id: str, payload: AddMembersByTagModel, coc_client: coc.Client = Depends(get_coc_client)):
    ...
    for member_input in payload.members:
        try:
            ...
            if not is_valid:
                ...
                errors.append(f"User {discord_user_id} already has {current_count}/{max_accounts} accounts in this roster")
                continue
...
@router.delete("/rosters/{roster_id}/members/{player_tag}", response_model=dict, status_code=status.HTTP_200_OK, name="Remove Member from Roster")
...
@router.patch("/rosters/{roster_id}/members/{player_tag}", response_model=dict, status_code=status.HTTP_200_OK, name="Update Member in Roster")
...
@router.post("/roster-templates/{template_id}/generate", response_model=dict, status_code=status.HTTP_201_CREATED, name="Generate Roster from Template")
    ...
    if event_time and frequency:
        ...
        if frequency == "weekly":
            ...
            if days_ahead == 0 and now.time() > event_dt.time():
                days_ahead = 7
            next_event = now.replace(hour=event_dt.hour, minute=event_dt.minute, second=0, microsecond=0).add(days=days_ahead)
        elif frequency == "monthly":
            # Next occurrence on same day of month
            try:
                next_event = now.replace(day=event_dt.day, hour=event_dt.hour, minute=event_dt.minute, second=0, microsecond=0)
                if next_event <= now:
                    next_event = next_event.add(months=1)
            except ValueError:  # Day doesn't exist in current month
                next_event = now.end_of('month').replace(hour=event_dt.hour, minute=event_dt.minute, second=0, microsecond=0)
                if next_event <= now:
                    next_event = next_event.add(months=1).end_of('month').replace(hour=event_dt.hour, minute=event_dt.minute, second=0, microsecond=0)
        else:  # cwl_season
...
@router.patch("/rosters/{roster_id}/phase", response_model=dict, status_code=status.HTTP_200_OK, name="Update Roster Phase")
async def update_roster_phase(roster_id: str, phase: str = Query(..., regex="^(scheduled|published|registration_closed|results_published)$")):
...
@router.post("/rosters/{roster_id}/refresh-members", response_model=dict, status_code=status.HTTP_200_OK, name="Refresh All Roster Members")
...
@router.post("/rosters/{roster_id}/members/refresh/{player_tag}", response_model=dict, status_code=status.HTTP_200_OK, name="Refresh Single Roster Member")
async def refresh_single_roster_member(roster_id: str, player_tag: str, coc_client: coc.Client = Depends(get_coc_client)):
...
@router.get("/rosters/{roster_id}/with-stats", response_model=dict, status_code=status.HTTP_200_OK, name="Get Roster with Calculated Stats")
...
@router.get("/rosters/{roster_id}/member-stats/{player_tag}", response_model=dict, status_code=status.HTTP_200_OK, name="Get Single Member Stats")
...
@router.get("/rosters/{roster_id}/user-stats", response_model=dict, status_code=status.HTTP_200_OK, name="Get User Account Statistics")
async def get_roster_user_stats(roster_id: str):
    ...
    return {
        ...
        "summary": {
            ...
            "users_at_limit": len([u for u in user_list if max_accounts and u["account_count"] >= max_accounts]) if max_accounts else 0,
...
@router.get("/rosters/{roster_id}/missing-members", response_model=dict, status_code=status.HTTP_200_OK, name="Get Missing Members for Single Roster")
