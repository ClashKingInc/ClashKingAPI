...
func TestStatsQueryRoutesUseQueryMethodAndJSONBody(t *testing.T) {
	...
	for _, path := range []string{"/v2/stats/armies", "/v2/stats/items", "/v2/stats/ranked", "/v2/stats/war", "/v2/stats/cwl"} {
		if registeredRouteIndex(app, statsQueryMethod, path) < 0 {
		...
		req := httptest.NewRequest(statsQueryMethod, path, nil)
	...
	req := httptest.NewRequest(statsQueryMethod, "/v2/stats/armies", strings.NewReader(`{}`))
	resp, err := app.Test(req)
	if err != nil {
		t.Fatal(err)
	}
