...
it("waits for a concurrent credit writer and allocates against its committed balance", async () => {
  await Effect.runPromise(Effect.gen(function* () {
    const sql = yield* SqlClient.SqlClient
    const sponsorId = '7934567890123456708'
    yield* sql`INSERT INTO servers (id, name) VALUES (${serverId}, 'AI usage fixture') ON CONFLICT DO NOTHING`
    yield* sql`INSERT INTO auth_users (user_id, provider) VALUES (${sponsorId}, 'discord')`
    yield* sql`INSERT INTO subscription_entitlements (user_id, active, roster_assistant_monthly_credit_usd) VALUES (${sponsorId}, true, 0.01)`
    const id = (yield* sql<{ id: string }>`INSERT INTO roster_ai_usage (server_id, discord_user_id, operation, provider, model)
      VALUES (${serverId}, ${userId}, 'view_or_membership_proposal', 'openai', 'gpt-5.6-luna') RETURNING id::text`)[0]!.id
    yield* sql`INSERT INTO roster_ai_usage_sponsors (usage_id, user_id, position) VALUES (${id}, ${sponsorId}, 0)`
    yield* Effect.promise(async () => {
      const ready = Promise.withResolvers<void>(), release = Promise.withResolvers<void>()
      const writer = Effect.runPromise(sql.withTransaction(Effect.gen(function* () {
        yield* lockRosterAIBudget(sql)
        yield* sql`UPDATE subscription_entitlements SET roster_assistant_monthly_credit_usd = 0.04 WHERE user_id = ${sponsorId}`
        ready.resolve()
      ...
      try {
        ...
        const settlement = Effect.runPromiseExit(settle(id).pipe(Effect.provideService(SqlClient.SqlClient, sql)))
          .then((result) => { settled = true; return result })
        await new Promise((resolve) => setTimeout(resolve, 30))
        expect(settled).toBe(false)
        release.resolve()
        const [, result] = await Promise.all([writer, settlement])
        expect(result._tag).toBe('Success')
        if (result._tag === 'Success') expect(result.value?.status).toBe(204)
      } finally {
        release.resolve()
        await writer
      }
    })
    expect(yield* sql`SELECT amount_usd::text FROM roster_ai_usage_credits WHERE usage_id = ${id}`)
      .toEqual([{ amount_usd: '0.02890000' }])
  }).pipe(Effect.provide(layer), Effect.scoped))
})
