...
const get = <P extends ContractSchema, Q extends ContractSchema, R extends ContractSchema>(
  operationId: string,
  path: `/v2/${string}`,
  auth: AuthMode,
  pathParams: P,
...
const mutate = <P extends ContractSchema, B extends ContractSchema, R extends ContractSchema>(
  operationId: string,
  method: "DELETE" | "POST",
  path: `/v2/${string}`,
  auth: AuthMode,
...
const forBot = <E extends AnyEndpoint, P extends ContractSchema>(
  endpoint: E, operationId: string, path: `/v2/${string}`, pathParams: P, summary: string,
) => {
  return defineEndpoint<P, E["query"], E["body"], E["response"], NonNullable<E["errors"]>>({
