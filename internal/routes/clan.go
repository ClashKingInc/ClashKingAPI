...
// @Produce json
// @Param clan_tag path string true "Clan tag"
// @Param body body models.ClanPlayerTagsBody false "Player tags"
// @Success 200 {object} map[string]interface{}
...
func boardTotals(a apptypes.Deps) fiber.Handler {
	return func(c *fiber.Ctx) error {
		var body models.ClanPlayerTagsBody
		if len(c.Body()) > 0 {
		...
		return apptypes.JSON(c, fiber.StatusOK, map[string]any{
			...
			"tracked_player_count": len(rows),
			"clan_games_points":    0,
			"troops_donated":       0,
			"troops_received":      0,
			"clan_capital_donated": 0,
			"activity":             len(rows),
		})
	}
}
...
func clanDonationsSingle(a apptypes.Deps) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		items := make([]map[string]any, 0, len(seasonData))
		for tag, raw := range seasonData {
			doc, _ := raw.(bson.M)
			items = append(items, map[string]any{"tag": tag, "donated": doc["donated"], "received": doc["received"]})
		}
...
// @Tags Clan
// @Produce json
// @Param body body models.ClanTagsBody true "Clan tags"
// @Success 200 {object} map[string]interface{}
// @Failure 400 {object} map[string]interface{}
// @Failure 500 {object} map[string]interface{}
