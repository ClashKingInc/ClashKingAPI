...
describe("Dashboard reminders", () => {
  ...
  it("commits a reminder without calling another service", async () => {
    const serverId = "1334567890123456789", channelId = "2334567890123456789"
    const events: string[] = []
    const query = (parts: TemplateStringsArray) => Effect.sync(() => {
      if (parts.join("").includes("pg_notify")) events.push("notify")
      return [{ id: serverId }]
    })
    ...
    expect(events).toEqual(["discord", "begin", "insert", "notify", "commit"])
  })
})
