...
/** Validate the final transaction state, never an unlocked admission estimate.
 * Includes substitutes. Removals need the same lock but do not call this guard,
 * so an imported overfull roster can still be reduced.
 */
...
export const assertRosterMembershipLimits = (sql: SqlClient.SqlClient, rosterIds: ReadonlyArray<string>, affectedOwnerIds?: ReadonlyArray<string>) => Effect.gen(function* () {
  yield* assertRosterCapacity(sql, rosterIds)
