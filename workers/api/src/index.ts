...
import { isTranscriptRequest, readTicketTranscript } from "./ticket-transcript-runtime.js"
...
export { TicketRuntimeCoordinator } from "./ticket-runtime-coordinator.js"
...
const liveLayer = (bindings: WorkerBindings) => {
  ...
  const core = Layer.mergeAll(authorization, databaseLayer(bindings))
...
export default {
  fetch(request: Request, bindings: WorkerBindings, context: ExecutionContext): Promise<Response> {
    ...
    if (isTranscriptRequest(request)) return readTicketTranscript(request, bindings.TICKETING)
    ...
    const program = Effect.suspend(() => route(request, bindings, (principal, proxyRequest, response) => {
      ...
      Effect.catchTag("SqlError", (cause) => Effect.fail(new DatabaseFailure({ cause, message: "Database unavailable" }))),
  ...
  scheduled(_controller: ScheduledController, bindings: WorkerBindings, context: ExecutionContext): void {
    ...
    context.waitUntil(bindings.RUNTIME_RECOVERY.getByName("persistent-runtime-recovery").wake())
  },
