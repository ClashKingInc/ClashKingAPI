...
func BuildDoc(scope Scope) (string, error) {
	...
	if scope == PublicScope {
		FilterPublicPaths(doc)
		delete(doc, "securityDefinitions")
	} else {
		FilterPrivatePaths(doc)
	}
...
func FilterPublicPaths(doc map[string]any) {
	filterPaths(doc, false)
}

func FilterPrivatePaths(doc map[string]any) {
	filterPaths(doc, true)
}
...
func setSwaggerMetadata(doc map[string]any, scope Scope) {
...
func filterPaths(doc map[string]any, authenticatedOnly bool) {
