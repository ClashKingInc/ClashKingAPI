...
async def calculate_player_activity(player_tag: str, days: int = 30) -> int:
    ...
    try:
        ...
        pipeline = [
            ...
            {"$group": {
                "_id": {"$dateToString": {"format": "%Y-%m-%d", "date": {"$toDate": {"$multiply": ["$time", 1000]}}}},
                "count": {"$sum": 1}
            }},
            {"$count": "total_days"}
        ]

        result = await mongo.player_history.aggregate(pipeline).to_list(length=1)
...
async def check_user_account_limit(roster_id: str, discord_user: str, exclude_tag: str = None) -> tuple[bool, int, int]:
    """
    ...
    for member in members:
        ...
        member_discord_id = extract_discord_user_id(member.get("discord", "No User"))
