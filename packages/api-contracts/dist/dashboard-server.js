...
export const ApproveMessages = Schema.Array(ApproveMessage).check(Schema.isMaxLength(25), Schema.makeFilter(messages => new Set(messages.map(message => message.name.trim())).size === messages.length ? undefined : "Template names must be unique after trimming"));
...
export const DiscordWebhookPayload = Schema.StructWithRest(Schema.Struct({
    ...
    embeds: Schema.optionalKey(Schema.Array(DiscordEmbed)),
    components: Schema.optionalKey(Schema.Array(JsonValue)),
    messages: Schema.optionalKey(Schema.Array(Schema.StructWithRest(Schema.Struct({ data: DiscordWebhookMessage }), [Schema.Record(Schema.String, JsonValue)]))),
