...
describe("final Legend ranking reads", () => {
  it("reads current ranks in requested order and omits unresolved optional identities", async () => {
    ...
    expect(query.mock.calls[0]?.[0]).toContain("FROM player_rankings_current source")
    expect(query.mock.calls[0]?.[0]).toContain("IN ('home','builder_base')")
    expect(query.mock.calls[0]?.[0]).not.toContain("player.location_id")
    expect(query.mock.calls[0]?.[0]).not.toContain("rank IS NOT NULL")
  ...
  it("reads compact historical snapshots and derives trophy buckets at read time", async () => {
    ...
    expect(history.query.mock.calls[0]?.[0]).toContain("legend_rankings_history")
    expect(history.query.mock.calls[0]?.[0]).not.toContain("leaderboard_history_player_home")
    expect(history.query.mock.calls[0]?.[0]).toContain("source.player_tag=history.tag")
    expect(history.query.mock.calls[0]?.[0]).toContain("IN ('home','builder_base')")
    expect(history.query.mock.calls[0]?.[0]).not.toContain("player.location_id")
