...
from routers.v2.clan.clan import get_clans_stats, get_clans_capital_raids
from routers.v2.clan.clan_models import ClanTagsRequest, RaidsRequest
...
from routers.v2.war.endpoints import (
    get_multiple_clan_war_summary,
    clan_warhits_stats,
    players_warhits_stats,
)
...
router = APIRouter(
    prefix='/v2/app', tags=['Mobile App'], include_in_schema=True
)
...
async def app_player_war_stats(body: PlayerTagsRequest) -> Dict[str, Any]:
    ...
    if not body.player_tags:
        raise HTTPException(
            status_code=400, detail='player_tags cannot be empty'
        )
...
@router.post(
    '/initialization', name='Initialize all account data for mobile app'
)
async def app_initialization(
    body: PlayerTagsRequest, request: Request
) -> Dict[str, Any]:
    """
    ...
    async def fetch_clan_war_logs(
        input_clan_tags: List[str],
    ) -> List[Dict[str, Any]]:
        """Fetch war logs using same logic as mobile app - direct API calls"""

        async def fetch_clan_war_log(
            http_session: aiohttp.ClientSession, clan_tag: str
        ) -> Dict[str, Any]:
        ...
        async with aiohttp.ClientSession() as war_session:
            return await asyncio.gather(
                *(
                    fetch_clan_war_log(war_session, tag)
                    for tag in input_clan_tags
                )
            )

    async def fetch_clan_war_stats(
        input_clan_request: ClanTagsRequest, req: Request
    ) -> Dict[str, Any]:
    ...
    async def fetch_players_basic_and_extended():
        ...
        async with aiohttp.ClientSession() as session:
            ...
            fetch_tasks = [
                fetch_player_api_data(session, tag) for tag in player_tags
            ]
        ...
        extended_results = await asyncio.gather(
            *[
                assemble_full_player_data(
                    tag, raid_data, war_data, mongo_data, tag_to_legends
                )
    ...
    async def fetch_clan_join_leave_data(
        input_clan_tags: List[str],
    ) -> Dict[str, Any]:
