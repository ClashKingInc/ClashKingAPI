...
func TestProxyForwardAllowsConfiguredBotToken(t *testing.T) {
	...
	app := fiber.New()
	deps := apptypes.Deps{Config: apptypes.Config{
		APIBotToken: "bot-secret",
		ProxyOrigin: upstream.URL + "/",
	}}
	rejectUserAuth := func(fiber.Handler) fiber.Handler {
	...
	app.Get(
		"/proxy/v1/*",
		authUserOrBot(deps, rejectUserAuth, proxyForward(deps, "/proxy/")),
	)
