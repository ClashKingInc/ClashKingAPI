...
const GroupName = Schema.String.check(Schema.isMinLength(1), Schema.isMaxLength(49), Schema.makeFilter(name =>
  name.trim().length ? undefined : "Roster member group name must not be blank"))
...
export const RosterMemberGroupSettingsRequest = Schema.Struct({ groups: Schema.Array(Schema.Struct({
  ...
  if (new Set(groups.map(group => group.member_group_id)).size !== groups.length) return "Roster member groups must be unique"
  if (groups.filter(group => group.signup_enabled).length > 24) return "At most 24 signup groups plus Main are supported"
  return undefined
})) })
