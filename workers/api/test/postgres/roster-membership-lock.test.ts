...
it("holds both proposed-tag and affected-owner mutexes until admission commits", async () => {
  await Effect.runPromise(Effect.gen(function* () {
    const sql = yield* SqlClient.SqlClient
    yield* sql`INSERT INTO player_links (tag, user_id, source) VALUES ('#8P2', '7834567890123456804', 'bot')`
    yield* sql`INSERT INTO player_link_mutation_locks (tag) VALUES ('#8P2')`
  ...
  try {
    await ready.promise
    for (const target of ['tag', 'subject'] as const) {
      const outcome = await Effect.runPromise(Effect.gen(function* () {
        const sql = yield* SqlClient.SqlClient
        return yield* sql.withTransaction(target === 'tag'
          ? sql`SELECT tag FROM player_link_mutation_locks WHERE tag = '#8P2' FOR UPDATE NOWAIT`
