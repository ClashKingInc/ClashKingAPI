...
import { DashboardUpdateRosterRequest, DashboardUpdateRosterMemberRequest } from "./dashboard-roster.js"
...
describe("retained roster member-group configuration", () => {
  ...
  it("keeps group/substitute distinct from old fields and accepts only null or positive per-user caps", () => {
    expect(Schema.is(DashboardUpdateRosterMemberRequest)({ member_group_id: group(1).member_group_id, is_substitute: true })).toBe(true)
    expect(Schema.is(DashboardUpdateRosterRequest)({ capacity: 50, roster_role_id: null, max_accounts_per_user: null })).toBe(true)
