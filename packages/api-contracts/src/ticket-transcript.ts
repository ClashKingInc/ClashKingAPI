...
const Timestamp = Schema.String.check(Schema.makeFilter(value => {
  ...
  if (!parts || !Number.isFinite(Date.parse(value))) return "Invalid timestamp"
...
export const TicketTranscriptDocument = Schema.Struct({
  ...
  if (Date.parse(document.captureStartedAt) > Date.parse(document.collectedAt)) return "Invalid capture interval"
  if (document.channels.reduce((sum, channel) => sum + channel.messages.length, 0) > MAX_TRANSCRIPT_MESSAGES) return "Transcript message limit exceeded"
  if (document.attachments.reduce((sum, attachment) => sum + attachment.size, 0) > MAX_TRANSCRIPT_ATTACHMENT_BYTES) return "Transcript attachment byte limit exceeded"
  ...
  if (channels.size !== document.channels.length || !channels.has(document.ticket.channelId)) return "Invalid transcript channels"
  if (attachments.size !== document.attachments.length) return "Duplicate transcript attachment"
  const messageIds = new Set<string>()
  const referencedAttachments = new Set<string>()
  for (const channel of document.channels) for (const message of channel.messages) {
    if (messageIds.has(message.id)) return "Duplicate transcript message"
    messageIds.add(message.id)
    if (message.attachmentIds.some(id => !attachments.has(id))) return "Missing transcript attachment reference"
    for (const id of message.attachmentIds) referencedAttachments.add(id)
  }
  if (referencedAttachments.size !== attachments.size) return "Unreferenced transcript attachment"
  return undefined
}))
