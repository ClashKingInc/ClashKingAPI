...
describe("canonical CWL archive coverage", () => {
  it("includes unknown league globally, gates missing coverage, and keeps totals across finalization", async () => {
    await Effect.runPromise(Effect.gen(function* () {
      ...
      expect(all.metrics.sampleSize).toBe(4)
      expect(all.metrics.averageStars).toBe(1.5)
      expect(all.metrics.averageDestruction).toBe(70)
      expect(all.metrics.threeStarRate).toBe(0.25)
      expect(all.breakdowns[0]!.key).toBe("2026-09")
      expect((yield* queryCwlStats({ dates })).metrics.sampleSize).toBe(0)
      expect((yield* queryCwlStats({ dates, equal_townhalls: false, townhall_level: 18 })).metrics.averageStars).toBe(0.5)
      ...
      expect((yield* queryCwlStats({ dates, seasons: ["2026-08"] })).metrics.sampleSize).toBe(0)
