...
const pair = (version = "0.1.0-rc.12") => [
...
test("requires one exact semantic version across the release package pair", () => {
  assert.equal(validateReleaseVersion(...pair()), "0.1.0-rc.12")
  const mismatch = pair(); mismatch[1].version = "0.1.0-rc.13"
  assert.throws(() => validateReleaseVersion(...mismatch), /same valid semantic version/u)
  const dependency = pair(); dependency[1].dependencies["@clashking/api-contracts"] = "^0.1.0-rc.12"
...
test("blocks unchanged pull-request versions and versions whose tag already exists", () => {
  assert.equal(validatePullRequestVersion(pair("0.1.0-rc.12"), pair("0.1.0-rc.11"), false), "0.1.0-rc.12")
  assert.equal(validatePullRequestVersion(pair("0.1.0-rc.12"), undefined, false), "0.1.0-rc.12")
