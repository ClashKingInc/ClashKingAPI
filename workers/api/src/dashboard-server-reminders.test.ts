...
describe("Dashboard reminders", () => {
  ...
  it.each([503, 200])("reports failed or unacknowledged Tracking publication (%s)", async (status) => {
    ...
    await expect(Effect.runPromise(publishReminderChange(bindings, { clan_tag: "", type: "War", action: "deleted", reminder_id: "01991412-3895-7000-8000-123456789012" }))).rejects.toMatchObject({ _tag: "UpstreamUnavailable" })
  })
